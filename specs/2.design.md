...
-   **`run.rs` (Core Logic)**
    ...
    -   **Details**: This is the heart of the application. It first compiles the string expressions from the configuration into `regex::Regex` objects. It then enters a loop, reading each line from the input stream. For each line, it applies each regex sequentially. This means the result of one substitution becomes the input for the next regex. It uses `regex::Regex::replace_all` for efficient, in-place string updates. This sequential approach ensures that substitutions can be chained, consistent with traditional text processing tools like `sed`.
